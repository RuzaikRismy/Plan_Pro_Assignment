...
const AutocompleteSearchField = React.forwardRef((props, ref) => {
  const {
    selectDropDownData,
    selectName,
    selectValue,
  ...
  AutocompleteSearchField.defaultProps = {
    selectDropDownData: [],
    autoComData: [],
    margin: "dense", //normal, dense, none
  };
    ...
    >
      ...
      {isAdvancedSearch && (
        <FormControl
          ...
          style={{ width: selectWidth, marginBottom: -4 }}
        >
          <Select
            labelId={`${comName}-autocomplete-select-label`}
            id={`${comName}-autocomplete-simple-label`}
            value={selectName}
      ...
      {isAutocompleteField ? (
        <Autocomplete
          ...
          getOptionDisabled={(option) => option[activeFieldName] === 0}
          renderOption={(option) => (
            <Typography>
              {option[visibleName1]} &nbsp; &nbsp; &nbsp; {option[visibleName2]}
            </Typography>
          )}
          style={{ width: "100%" }}
          renderInput={(params) => (
